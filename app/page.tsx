...
export default function HomePage() {
  ...
  async function uploadSelectedAssets(projectId: string) {
    ...
    for (let index = 0; index < assets.length; index += 1) {
      ...
      if (!signResponse.ok) {
        if (signed?.status === 'not_configured' || signResponse.status === 503) {
          setGeneration({ loading: false, message: 'Cloud asset storage is not connected yet. Using browser preview assets for now.' });
  ...
  async function generateMovingVideo() {
    ...
    try {
      ...
      const providerProject = {
        ...
        title: project.title,
        scenes: project.scenes.map((scene) => ({ id: scene.id, title: scene.title, durationSeconds: scene.duration, visualPrompt: scene.visualPrompt, camera: scene.camera, voiceText: scene.narration, transition: scene.transition, referenceAssetUrls: assetUrls })),
      };
      setGeneration({ loading: true, message: 'Starting GPU generation…', progress: 15 });
      const response = await fetch('/api/video/generate', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ project: providerProject }) });
      const data = (await response.json()) as ProviderJob;
      if (!response.ok) {
        const providerMissing = data?.status === 'not_configured' || response.status === 503;
        setGeneration({ loading: false, message: providerMissing ? 'GPU video provider is not connected yet. Browser motion export is available now.' : data?.error || 'Video generation could not start.', jobId: data?.id, progress: data?.progress || 0 });
        return;
      }
  ...
  return (
    <main className="shell">
      <section className="hero"><div className="badge">AI VIDEO MAKER • BUILD 08</div><h1>Story to real moving video</h1><p>Build a proper motion-video project from your story, logo, app screens and references. The goal is cinematic motion and professional promo editing — not a photo slideshow.</p></section>
      <SystemReadiness />
      ...
      {project && <section className="storyboard" id="storyboard"><div className="storyHeader"><div><div className="badge">SCENE ENGINE READY</div><h2>{project.title}</h2><p>{project.scenes.length} scenes • {project.totalDuration}s • {project.format}</p></div><button className="secondary" onClick={downloadProject}>Download Project JSON</button></div><PromoPreview project={project} assets={assets} /><BrowserVideoExporter project={project} assets={assets} /><div className="timeline">{project.scenes.map((scene) => <article className="sceneCard" key={scene.id}><div className="sceneTop"><div className="sceneNumber">{scene.order}</div><div><h3>{scene.title}</h3><span>{scene.duration}s</span></div></div><div className="sceneGrid"><div><b>Visual</b><p>{scene.visualPrompt}</p></div><div><b>Camera</b><p>{scene.camera}</p></div><div><b>Voice</b><p>{scene.narration}</p></div><div><b>Transition</b><p>{scene.transition}</p></div>{scene.onScreenText && <div className="wide"><b>On-screen text</b><p>{scene.onScreenText}</p></div>}</div></article>)}</div><div className="generationBox"><div><strong>AI moving-video engine</strong><p>GPU provider connect ஆனதும் every scene AI clip + voice + subtitles + final video automatically generate ஆகும்.</p></div><button className="primary" type="button" disabled={generation.loading} onClick={generateMovingVideo}>{generation.loading ? 'Generating…' : 'Generate AI Moving Video'}</button>{generation.message && <div className="generationStatus">{generation.message}</div>}{typeof generation.progress === 'number' && generation.loading && <div className="progressTrack"><div className="progressFill" style={{ width: `${generation.progress}%` }} /></div>}{generation.outputUrl && <a className="secondaryLink" href={generation.outputUrl} target="_blank" rel="noreferrer">Open generated video</a>}</div></section>}
    </main>
  );
}
